  ...
  <div>
    ...
    <q-form @reset="onReset" class="q-gutter-md q-pa-sm">
      <q-card class="my-card q-pa-sm q-pr-sm q-pb-sm" flat bordered>
        <q-select v-model="login" option-label="name" option-value="number" outlined class="col" use-input hide-selected
          bottom-slots fill-input dense input-debounce="0" :options="options" @filter="filterFn" label="Сотрудник"
          :behavior="$q.platform.is.ios === true ? 'menu' : 'menu'">
          <template v-slot:hint>
            <div class="text-primary">{{ login.division }}</div>
          </template>
          ...
          <template v-slot:append>
            <q-icon name="close" @click.stop="login = ''" class="cursor-pointer"></q-icon>
          </template>
      ...
      <q-card class="my-card q-pa-sm q-pr-sm q-pb-sm hidden" flat bordered>
        <q-input dense outlined readonly v-model="login.division" label="Подразделение" hint="Подразделение">
      ...
      <q-card class="my-card q-pa-sm q-pr-sm q-pb-md" flat bordered>
        <q-input @update:model-value="(val) => checkOfFilling(val)" dense outlined v-model="login.tel" type="textarea"
...
export default defineComponent({
  ...
  methods: {
    ...
    filterFn(val, update) {
      console.log(`Длина - ${this.userList.length}`);
      if (val.length < 2) {
        return;
      }
      update(() => {
    ...
    async sendInquiry() {
      ...
      try {
        return await uni_rersponse2(dataForm, dataForm.nameMethod)
          .then(res => {
            console.log(res);
          })
          .then(() => { });
    ...
    //     spinner: QSpinnerGears,
    //     backgroundColor: "bg",
    //     message: "Обновление справочников...",
    //   });
    //   try {
    //     return await uni_rersponse2(dataForm, dataForm.nameMethod)
    //       .then((res) => {
